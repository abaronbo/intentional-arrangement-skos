...
Collections export as first-class citizens: `skos:prefLabel` for the name (one per language; extra same-language names become `skos:altLabel`), `skos:note` for the note, `skos:inScheme`, and `dcterms:created`/`modified` date stamps — the same description discipline concepts get. Editing a collection — its name, note, type, or members — also records a dated **change note** that exports as `skos:changeNote`, the same edit-history trail concepts keep, so a published collection carries not just *when* it changed but *what* changed. Collections created before v0.17.0 carry no dates and none are invented for them — the editor never fabricates provenance; such a collection gains a `dcterms:modified` stamp the first time you actually edit it, and `dcterms:created` stays absent by design.
...
- **What it checks** — one preferred label per language, label disjointness, no cycles in the hierarchy, no orphans, `related`/`broader` clashes, undocumented concepts, and more. It also enforces the SKOS **class-disjointness** conditions — one URI can't be both a `skos:Concept` and the `skos:ConceptScheme` (S9) or a `skos:Collection` (S37) — and the **mapping-clash** condition — `skos:exactMatch` must not point at the same target as `broadMatch`/`narrowMatch`/`relatedMatch` (S46). The full list, with what each means, is in the [SKOS reference](skos-reference.md#the-conditions-that-keep-it-sound).
- **Label disjointness is exact.** SKOS (S13) makes `prefLabel`, `altLabel` and `hiddenLabel` pairwise disjoint per *exact literal* — same text **and** language. Case matters: an `altLabel` "hound"@en and a `hiddenLabel` "Hound"@en are different literals and perfectly legal (a common way to keep a capitalisation variant findable in search). Only the identical string under two label properties is flagged, and the one-click fix removes only true duplicates — never a case or spacing variant.
